...
HRESULT CRanking::Init()
{
	...
	CPressAnyButton::Create(D3DXVECTOR3(SCREEN_WIDTH - 150.0f, SCREEN_HEIGHT - 40.0f, 0.0f), 150.0f, 70.0f, CPressAnyButton::TYPE_ANY);
...
void CRanking::Update(void)
{
	...
	if (m_bSetBlock == true)
	{
		...
		if (m_nBlockTimer > 1)
		{
			if (m_nCntBlock < m_nNumModel)
			{// ランキングロゴ
				// ブロック生成
				CBlock::Create(D3DXVECTOR3(m_BlockPos[m_nCntBlock].x, m_BlockPos[m_nCntBlock].y + 1000.0f , m_BlockPos[m_nCntBlock].z), CBlock::STATE_NORMAL);
				m_nCntBlock++;
				m_nBlockTimer = 0;
			}
		}
	...
	if (m_bSetNumBlock == true)
	{
		m_nBlockNumTimer++;
		if (m_nBlockNumTimer > 1)
		{
			if (m_nCntNumBlock < m_nNumRankModel)
			{// 順位ロゴ
			 // ブロック生成
				CBlock::Create(D3DXVECTOR3(m_BlockNumPos[m_nCntNumBlock].x - 200.0f, m_BlockNumPos[m_nCntNumBlock].y + 1000.0f, m_BlockNumPos[m_nCntNumBlock].z + 3000.0f), CBlock::STATE_NORMAL);
				m_nCntNumBlock++;
				m_nBlockNumTimer = 0;

				if (m_nCntNumBlock % 10 == 0)
				{
