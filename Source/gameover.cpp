...
HRESULT CGameOver::Init(void)
{
	...
	for (int nCntCharX = 0; nCntCharX < NUM_X; nCntCharX++)
	{
		for (int nCntCharY = 0; nCntCharY < NUM_Y; nCntCharY++)
		{
			m_pBlock[nCnt] = CBlock::Create(D3DXVECTOR3(-300.0f + 50.0f * nCntCharX, 50.0f * nCntCharY, -650.0f), CBlock::STATE_NORMAL);
			nCnt++;
		}
	}
			...
			while (1)
			{
				...
				if (memcmp(&aStr[0], "NUM_MODEL =", strlen("NUM_MODEL =")) == 0)
				{
					...
					for (int nCntModel = 0; nCntModel < m_nNumModel; nCntModel++)
					{
						...
						// ブロック生成
						CBlock::Create(D3DXVECTOR3(m_BlockPpos[nCntModel].x - 300.0f, m_BlockPpos[nCntModel].y, m_BlockPpos[nCntModel].z - 650.0f), CBlock::STATE_NORMAL);
					}
				}
