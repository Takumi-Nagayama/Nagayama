...
public:
	...
	void CollisonWoodBlock(D3DXVECTOR3 *pos, float fRadius);											// ブロックの破壊の判定
	void CollisonCoin(D3DXVECTOR3 *pos, float fRadius);											// コインの当たり判定
	void CollisonGem(D3DXVECTOR3 *pos, float fRadius);											// 宝石の当たり判定
	...
	void CollisionSceneObj(D3DXVECTOR3 *pos, D3DXVECTOR3 *posOld, D3DXVECTOR3 *move, D3DXVECTOR3 radius);	// 背景オブジェクトとの当たり判定
