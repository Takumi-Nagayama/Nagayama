...
#define RESPAWN_POS				(D3DXVECTOR3(80.0f, 50.0f, 250.0f))		// リスポーン位置
...
void CPlayer::CollisonModel(D3DXVECTOR3 *pos, D3DXVECTOR3 *posOld, D3DXVECTOR3 *move, D3DXVECTOR3 radius)
{
	...
	{// 優先順位が3のオブジェクトを1つ1つ確かめる
	 // 処理の最中に消える可能性があるから先に記録しておく
		...
		{// 死亡フラグが立っていないもの
			if (pScene->GetObjType() == CScene::OBJTYPE_BLOCK || pScene->GetObjType() == CScene::OBJTYPE_IRONBLOCK || pScene->GetObjType() == CScene::OBJTYPE_WOODBLOCK)
			{// オブジェクトの種類を確かめる
				if (((CBlock*)pScene)->GetState() != ((CBlock*)pScene)->STATE_PLAYER)
				{
...
// モデルとの当たり判定の処理
//=============================================================================
void CPlayer::CollisionSceneObj(D3DXVECTOR3 *pos, D3DXVECTOR3 *posOld, D3DXVECTOR3 *move, D3DXVECTOR3 radius)
{
	CScene *pScene = NULL;

	// 先頭のオブジェクトを取得(モデルの優先順位が2だから、2にあるオブジェクトをすべて見る)
	pScene = CScene::GetTop(SCENEOBJ_PRIORITY);

	while (pScene != NULL)
	{// 優先順位が2のオブジェクトを1つ1つ確かめる
	 // 処理の最中に消える可能性があるから先に記録しておく
		CScene *pSceneNext = pScene->GetNext();
		...
		{// 死亡フラグが立っていないもの
			...
			{// オブジェクトの種類を確かめる
				...
				m_bLand = ((CSceneObject*)pScene)->Collision(pos, posOld, move, radius);
			}
		}
...
// ブロックの破壊の判定
//=============================================================================
void CPlayer::CollisonBlock(D3DXVECTOR3 *pos, float fRadius)
{
	// モードの取得
	CManager::MODE mode;
	mode = CManager::GetMode();
	...
	{// 優先順位が3のオブジェクトを1つ1つ確かめる
	 // 処理の最中に消える可能性があるから先に記録しておく
		...
		{// 死亡フラグが立っていないもの
			...
			{// オブジェクトの種類を確かめる
				bool bDestroy = ((CBlock*)pScene)->CollisionBreak(pos, fRadius);

				if (((CBlock*)pScene)->CollisionBreak(pos, fRadius) == true)
				{
					if (m_nKey == ATTACK_KEY)
					{
						pSound->PlaySound(CSound::SOUND_LABEL_SE_BREAK);

						// ブロック削除
						((CBlock*)pScene)->Uninit();
						...
						for (int nCntParticle = 0; nCntParticle < BREAK_PARTICLE_NUM; nCntParticle++)
						{
							CParticleX::Create(D3DXVECTOR3(((CBlock*)pScene)->GetPosition().x, ((CBlock*)pScene)->GetPosition().y + 30.0f, ((CBlock*)pScene)->GetPosition().z),
...
// 鉄ブロックの破壊の判定
//=============================================================================
void CPlayer::CollisonIronBlock(D3DXVECTOR3 *pos, float fRadius)
{
	// モードの取得
	CManager::MODE mode;
	mode = CManager::GetMode();
	...
	{// 優先順位が3のオブジェクトを1つ1つ確かめる
	 // 処理の最中に消える可能性があるから先に記録しておく
		...
		if (pScene->GetDeath() == false)
		{// 死亡フラグが立っていないもの
			if (pScene->GetObjType() == CScene::OBJTYPE_IRONBLOCK)
			{// オブジェクトの種類を確かめる
				bool bDestroy = ((CIronBlock*)pScene)->CollisionBreak(pos, fRadius);

				if (((CIronBlock*)pScene)->CollisionBreak(pos, fRadius) == true)
				{
					if (m_nKey == ATTACK_KEY && m_bIronSound == true)
					{
						pSound->PlaySound(CSound::SOUND_LABEL_SE_IRON);
						m_bIronSound = false;

						for (int nCntParticle = 0; nCntParticle < IRON_PARTICLE_NUM; nCntParticle++)
						{
							CParticle3D::Create(POS_HAND,
								D3DXVECTOR3(sinf(PARTICLE_ROT) * ((rand() % 5 + 1)), cosf(PARTICLE_ROT) * ((rand() % 5 + 1)), cosf(PARTICLE_ROT) * ((rand() % 5 + 1))),
								D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f),
								IRON_PARTICLE_SIZE,
								IRON_PARTICLE_SIZE,
...
// ブロックの破壊の判定
//=============================================================================
void CPlayer::CollisonWoodBlock(D3DXVECTOR3 *pos, float fRadius)
{
	// モードの取得
	CManager::MODE mode;
	mode = CManager::GetMode();
	...
	{// 優先順位が3のオブジェクトを1つ1つ確かめる
	 // 処理の最中に消える可能性があるから先に記録しておく
		...
		if (pScene->GetDeath() == false)
		{// 死亡フラグが立っていないもの
			if (pScene->GetObjType() == CScene::OBJTYPE_WOODBLOCK)
			{// オブジェクトの種類を確かめる
				bool bDestroy = ((CWoodBlock*)pScene)->CollisionBreak(pos, fRadius);

				if (((CWoodBlock*)pScene)->CollisionBreak(pos, fRadius) == true)
				{
					if (m_nKey == ATTACK_KEY)
					{
						pSound->PlaySound(CSound::SOUND_LABEL_SE_BREAK);
						...
						// ブロック削除
						((CWoodBlock*)pScene)->Uninit();

						for (int nCntParticle = 0; nCntParticle < WOOD_PARTICLE_NUM; nCntParticle++)
						{
							CParticleX::Create(D3DXVECTOR3(((CWoodBlock*)pScene)->GetPosition().x, ((CWoodBlock*)pScene)->GetPosition().y + 30.0f, ((CWoodBlock*)pScene)->GetPosition().z),
								D3DXVECTOR3(sinf(D3DX_PI * PARTICLE_ROT), cosf(D3DX_PI * PARTICLE_ROT), cosf(D3DX_PI * PARTICLE_ROT)),
								D3DXVECTOR3(sinf(PARTICLE_ROT) * ((rand() % 7 + 1)), cosf(PARTICLE_ROT) * ((rand() % 7 + 1)), cosf(PARTICLE_ROT) * ((rand() % 7 + 1))),
								WOOD_PARTICLE_LIFE,
								CParticleX::TYPE_DOWN);

							CParticleX::Create(D3DXVECTOR3(((CWoodBlock*)pScene)->GetPosition().x, ((CWoodBlock*)pScene)->GetPosition().y + 60.0f, ((CWoodBlock*)pScene)->GetPosition().z),
								D3DXVECTOR3(sinf(D3DX_PI * PARTICLE_ROT), cosf(D3DX_PI * PARTICLE_ROT), cosf(D3DX_PI * PARTICLE_ROT / 100.0f)),
								D3DXVECTOR3(sinf(PARTICLE_ROT) * ((rand() % 7 + 1)), cosf(PARTICLE_ROT) * ((rand() % 7 + 1)), cosf(PARTICLE_ROT) * ((rand() % 7 + 1))),
								WOOD_PARTICLE_LIFE,
								CParticleX::TYPE_DOWN);
						}
...
// 宝石の当たり判定
//=============================================================================
void CPlayer::CollisonGem(D3DXVECTOR3 *pos, float fRadius)
{
	// モードの取得
	CManager::MODE mode;
	mode = CManager::GetMode();
	...
	// 先頭のオブジェクトを取得(モデルの優先順位が3だから、3にあるオブジェクトをすべて見る)
	pScene = CScene::GetTop(GEM_PRIORITY);
	...
	{// 優先順位が3のオブジェクトを1つ1つ確かめる
	 // 処理の最中に消える可能性があるから先に記録しておく
		...
		if (pScene->GetDeath() == false)
		{// 死亡フラグが立っていないもの
			if (pScene->GetObjType() == CScene::OBJTYPE_GEM)
			{// オブジェクトの種類を確かめる
				bool bHit = ((CGem*)pScene)->Collision(pos, fRadius);

				if (((CGem*)pScene)->Collision(pos, fRadius) == true)
				{
					pSound->PlaySound(CSound::SOUND_LABEL_SE_COIN);
					...
					pScore->AddScore(SCORE_GEM);

					for (int nCntParticle = 0; nCntParticle < GEM_PARTICLE_NUM; nCntParticle++)
					{
						CParticle3D::Create(D3DXVECTOR3(((CSceneX*)pScene)->GetPosition().x, ((CSceneX*)pScene)->GetPosition().y - 10.0f, ((CSceneX*)pScene)->GetPosition().z),
							D3DXVECTOR3(sinf(PARTICLE_ROT) * ((rand() % 5 + 1)), cosf(PARTICLE_ROT) * ((rand() % 5 + 1)), cosf(PARTICLE_ROT / 100.0f) * ((rand() % 5 + 1))),
