...
// 静的メンバ変数宣言
//=============================================================================
LPD3DXMESH CParticleX::m_pMesh = NULL;		// メッシュ情報（頂点情報）へのポインタ
LPD3DXBUFFER CParticleX::m_pBuffMat = NULL;	// マテリアル情報へのポインタ
DWORD CParticleX::m_nNumMat = 0;			// マテリアル情報の数
...
CParticleX *CParticleX::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, D3DXVECTOR3 move, int nLife, PARTICLE_TYPE type)
{
	...
	if (pParticle == NULL)
	{
		...
		if (pParticle != NULL)
		{
			...
			pParticle->BindModel(m_pBuffMat, m_nNumMat, m_pMesh);
...
// ブロックのモデル読み込み処理
//=============================================================================
HRESULT CParticleX::LoadModel(void)
{
	// レンダラーを取得
	CRenderer *pRenderer;
	pRenderer = CManager::GetRenderer();
	...
	// Xファイルの読み込み
	D3DXLoadMeshFromX(BLOCKDEBRIS_NAME,
		D3DXMESH_SYSTEMMEM,
		pDevice,
		NULL,
		&m_pBuffMat,
...
// ブロックのモデル解放処理
//=============================================================================
void CParticleX::UnloadModel(void)
{
	// メッシュの開放
	if (m_pMesh != NULL)
	{
